...
const PersonalityWindow = ({ state }) => {
    ...
    const getBorderColor = () => {
        if (viewState.affection > 50) return 'border-pink-500/50 shadow-[0_0_30px_rgba(236,72,153,0.15)]';
        if ((viewState.mood || '').includes('happy')) return 'border-green-500/50 shadow-[0_0_30px_rgba(34,197,94,0.15)]';
        return 'border-white/[0.14] shadow-2xl';
    };

    return (
        <div className={`fixed top-[64px] left-4 z-[60] w-80 bg-black/50 backdrop-blur-2xl border ${getBorderColor()} rounded-2xl p-5 transition-all duration-500 select-none pointer-events-none`}>
            <div className="flex items-center justify-between mb-4 border-b border-white/10 pb-3">
                <span className="text-xs font-bold tracking-[0.2em] text-white/60 uppercase">{t('personality.state')}</span>
                <div className={`w-2 h-2 rounded-full ${viewState.energy > 0.5 ? 'bg-green-500 shadow-[0_0_8px_rgba(34,197,94,0.8)]' : 'bg-yellow-500 shadow-[0_0_8px_rgba(234,179,8,0.8)]'} animate-pulse`} />
            </div>
            ...
            <div className="space-y-4">
                ...
                <div className="flex items-center justify-between group">
                    <div className="flex items-center gap-3 text-pink-400/90 group-hover:text-pink-400 transition-colors">
                        <div className="p-1.5 rounded-lg bg-pink-500/10 border border-pink-500/20">
                            <Heart size={14} className={viewState.affection > 20 ? "fill-pink-500/50" : ""} />
                        </div>
                    ...
                    <span className="text-[10px] font-bold text-pink-300 font-mono tracking-tighter">
                        {viewState.affection_hearts || viewState.affection.toFixed(1)}
                    </span>
                </div>
                ...
                <div className="space-y-2">
                    ...
                    <span className="text-xs font-mono text-blue-300">{Math.round(viewState.energy * 100)}%</span>
                    </div>
                    <div className="w-full h-1.5 bg-white/5 rounded-full overflow-hidden border border-white/5">
                        <div 
                            className="h-full bg-gradient-to-r from-blue-600 to-blue-400 transition-all duration-500 rounded-full"
                            style={{ width: `${Math.round(viewState.energy * 100)}%` }}
                        />
                    </div>
                </div>
                ...
                <div className="pt-3 border-t border-white/10">
                    ...
                    <div className="flex justify-between items-center">
                        <div className="flex items-baseline gap-1">
                            <span className="text-sm font-bold text-white/90">{t('personality.day')} {viewState.cycle_day}</span>
                            <span className="text-xs text-white/30 font-mono">/ {viewState.current_cycle_length}</span>
                        </div>
