...
_CAPABILITY_REQUEST_RE = re.compile(
    ...
    r"turn\s+off|start|launch|send|create|check|search|find|download|read|take|write|wake"
    r")\b.*\b("
    ...
    r"reminder|calendar|event|schedule|plan|note|notes|file|folder|music|weather|light|device"
    r")\b.*\b("
    r"ustaw\w*|dodaj\w*|dopisz\w*|zanotuj\w*|usuń\w*|usun\w*|anuluj\w*|zapisz\w*|włącz\w*|wlacz\w*|wyłącz\w*|wylacz\w*|stwórz\w*|stworz\w*|zrób\w*|zrob\w*|"
    r"set|add|delete|remove|cancel|save|turn\s+on|turn\s+off|create"
    r")\b",
    re.IGNORECASE,
)
